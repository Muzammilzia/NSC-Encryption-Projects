...
export const decrypt = (inputText: string, shift: number) => {
    ...
    for (let i = 0; i < textArray.length; i++) {
        ...
        for (let j = 0; j < letters.length; j++) {
            if(textArray[i] === letters[j]){
                ...
                const index = j - shift < 0 ? j - shift + 25 : j - shift;
...
// by using ascii code
export const decryptWithAscii = (inputText: string, shift: number) => {
    const textArray = inputText.split('');
    const outputArray: string[] = [];
    ...
    for (let i = 0; i < textArray.length; i++) {
        ...
        if(currentAscii >= 97 && currentAscii <= 122){
            const newAscii = currentAscii - shift < 97 ? currentAscii - shift + 25 : currentAscii - shift;
